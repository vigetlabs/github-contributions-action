...
function aggregateResults(projectStats, userStats, user, openedPrs, mergedPrs, reviews) {
...
function ensureProject(projectStats, project) {
...
Object.defineProperty(exports, "__esModule", ({ value: true }));
exports.pullRequestReviews = exports.pullRequestsMerged = exports.pullRequestsOpened = exports.queryAllMetrics = void 0;
/**
...
const queryAllMetrics = async (token, author, dateRange, logger) => {
    const [openedPrs, mergedPrs, reviews] = await Promise.all([
        (0, exports.pullRequestsOpened)(token, author, dateRange, logger),
        (0, exports.pullRequestsMerged)(token, author, dateRange, logger),
        (0, exports.pullRequestReviews)(token, author, dateRange, logger),
    ]);
...
exports.queryAllMetrics = queryAllMetrics;
const pullRequestsOpened = async (token, author, dateRange, logger) => {
    return query(`https://api.github.com/search/issues?q=type:pr+author:${author}`, token, dateRange, logger);
};
exports.pullRequestsOpened = pullRequestsOpened;
const pullRequestsMerged = async (token, author, dateRange, logger) => {
    return query(`https://api.github.com/search/issues?q=type:pr+is:merged+author:${author}`, token, dateRange, logger);
};
exports.pullRequestsMerged = pullRequestsMerged;
const pullRequestReviews = async (token, commenter, dateRange, logger) => {
    return query(`https://api.github.com/search/issues?q=type:pr+commenter:${commenter}+-author:${commenter}`, token, dateRange, logger);
};
exports.pullRequestReviews = pullRequestReviews;
/**
 ...
 * giving up entirely).
 */
async function query(url, token, [startDate, endDate], logger) {
    ...
    while (nextUrl && lastPage >= page) {
        ...
        if (links) {
            lastPage = Number(links.last?.page || /* istanbul ignore next */ lastPage);
...
async function queryPage(url, token, logger, retries = 0) {
    ...
    return {
        links: (0, parse_link_header_1.default)(response.headers.get('Link')),
        data: json,
    };
}
function exceededRateLimit(response) {
    return (response.status === 403 &&
        response.headers.get('x-ratelimit-remaining') === '0');
}
async function waitForRateLimitReset(response, logger) {
    ...
    logger.debug(`Rate limit exceeded. Waiting ${waitTimeMs / 1000} seconds to try again...`);
...
Object.defineProperty(exports, "__esModule", ({ value: true }));
exports.getMostActiveProject = exports.getMostActiveUser = void 0;
/**
 * User activity is measured by the number of opened PRs and reviews.
 */
function getMostActiveUser(userStats) {
...
function getMostActiveProject(projectStats) {
    ...
    return [mostActiveProject, mostActiveProjectCount];
}
...
Object.defineProperty(exports, "__esModule", ({ value: true }));
exports.collectActivity = void 0;
const date_fns_1 = __nccwpck_require__(3314);
const aggregate_1 = __nccwpck_require__(2686);
const api_1 = __nccwpck_require__(7272);
const awards_1 = __nccwpck_require__(80);
async function collectActivity(inputs, logger) {
    const { githubToken, startDate, endDate, users } = inputs;
    const dateRange = [startDate, endDate];
    const projectStats = {};
    const userStats = Object.fromEntries(users.map(username => [
        username,
        {
            opened: 0,
            merged: 0,
    ...
    for (const user of users) {
        const { openedPrs, mergedPrs, reviews } = await (0, api_1.queryAllMetrics)(githubToken, user, dateRange, logger);
        (0, aggregate_1.aggregateResults)(projectStats, userStats, user, openedPrs, mergedPrs, reviews);
        totalOpenedPrs += openedPrs.total_count;
        totalMergedPrs += mergedPrs.total_count;
        totalReviews += reviews.total_count;
    }
    const [mostActiveUser, mostActiveUserCount] = (0, awards_1.getMostActiveUser)(userStats);
    const [mostActiveProject, mostActiveProjectCount] = (0, awards_1.getMostActiveProject)(projectStats);
    const start = (0, date_fns_1.format)(startDate, 'MMM d, yyyy');
    const end = (0, date_fns_1.format)(endDate, 'MMM d, yyyy');
    ...
    logger.debug(`👑 Most Active Project: ${mostActiveProject} with ${mostActiveProjectCount} contributions`);
    logger.debug(`🥇 Most Active User: ${mostActiveUser} with ${mostActiveUserCount} contributions`);
    return {
        start,
        end,
        totalOpenedPrs,
...
exports.collectActivity = collectActivity;
...
Object.defineProperty(exports, "__esModule", ({ value: true }));
exports.createSlackBlocks = void 0;
...
function createSlackBlocks(activity, logger) {
    const { start, end, totalOpenedPrs, totalMergedPrs, totalReviews, mostActiveProject, mostActiveProjectCount, mostActiveUser, mostActiveUserCount, } = activity;
    const slackBlocks = [
        {
...
Object.defineProperty(exports, "__esModule", ({ value: true }));
exports.error = exports.warning = exports.notice = exports.info = exports.debug = exports.isDebug = void 0;
const core = __importStar(__nccwpck_require__(2186));
/* istanbul ignore next */
function isDebug() {
    return core.isDebug();
...
const collect_1 = __nccwpck_require__(9811);
const slack_blocks_1 = __nccwpck_require__(3995);
/**
...
async function run() {
    try {
        ...
        const activity = await (0, collect_1.collectActivity)(inputs, logger);
        core.debug(`Finished collecting data... ${new Date().toTimeString()}`);
        const slackBlocks = (0, slack_blocks_1.createSlackBlocks)(activity, logger);
